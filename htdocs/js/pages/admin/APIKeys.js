// SpeechBubble Admin Page -- API Keys
...
Class.add( Page.Admin, {
	...
	get_api_key_edit_html: function() {
		...
		// web hook url
		html += get_form_table_row( 'Web Hook URL', '<input type="text" id="fe_ak_web_hook_url" size="50" value="'+escape_text_field_value(api_key.web_hook_url)+'" spellcheck="false" placeholder="https://"/>' );
		html += get_form_table_caption( "Optionally enter a URL to invoke via JSON REST for all standard messages posted in public channels.<br/>These can optionally be filtered by using a Web Hook Filter (below).");
		html += get_form_table_spacer();
		
		// web hook filter
		html += get_form_table_row( 'Web Hook Filter', '<input type="text" id="fe_ak_web_hook_filter" size="30" value="'+escape_text_field_value(api_key.web_hook_filter)+'" spellcheck="false" placeholder="^\\!mycommand"/>' );
		html += get_form_table_caption( "If specified, only chat messages whose content matches this regular expression will be hooked.");
		html += get_form_table_spacer();
	...
	get_api_key_form_json: function() {
		...
		api_key.web_hook_url = $('#fe_ak_web_hook_url').val();
		api_key.web_hook_filter = $('#fe_ak_web_hook_filter').val();
