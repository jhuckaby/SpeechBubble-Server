...
Class.add( Page.Admin, {
	...
	receive_users: function(resp) {
		...
		var cols = ['Username', 'Nickname', 'Full Name', 'Email Address', 'Status', 'Type', 'Created', 'Actions'];
		...
		html += '<div class="subtitle">';
			...
			html += '<div class="subtitle_widget"><i class="fa fa-search">&nbsp;</i><input type="text" id="fe_ul_search" size="15" placeholder="Find username..." style="border:0px; background:transparent;"/></div>';
		...
		html += this.getPaginatedTable( resp, cols, 'user', function(user, idx) {
			...
			return [
				...
				'<div class="ellip" style="max-width:'+col_width+'px;">' + user.nickname + '</div>',
		...
		$.ajax({
			...
			success: function(data){
				...
				if (data.results && data.results[0] && data.results[0]) {
					...
					$('#fe_eu_nickname').val( user.name.first );
	...
	do_new_user: function(force) {
		...
		if (!user.nickname.length) {
			return app.badField('#fe_eu_nickname', "Please enter a nickname for the new account.");
		}
	...
	receive_user: function(resp) {
		...
		html += get_form_table_row( 'Avatar', '<input id="fe_eu_avatar" type="file" accept="image/*">' );
		html += get_form_table_caption( "Optionally add or replace the avatar image for the user.");
		html += get_form_table_spacer();
	...
	do_save_user: function() {
		...
		// ZU will call do_save_user_continue() upon completion.
		var files = this.div.find('#fe_eu_avatar').get(0).files;
		if (files.length) {
			// need upload
	...
	do_save_user_continue: function() {
		// avatar complete, save user now
	...
	get_user_edit_html: function() {
		...
		// nicknamne
		html += get_form_table_row( 'Nickname', '<input type="text" id="fe_eu_nickname" size="15" value="'+escape_text_field_value(user.nickname)+'" spellcheck="false"/>' );
		html += get_form_table_caption( "User's nickname, for display purposes in chats.");
		html += get_form_table_spacer();
	...
	get_user_form_json: function() {
		...
		var user = {
			...
			nickname: trim($('#fe_eu_nickname').val()),
		...
		// nickname is required, default to username
