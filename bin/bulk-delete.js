...
// Simple command-line bulk delete for Unbase Speech
// NOTE: DO NOT USE THIS WHILE SERVER IS ACTIVE -- could run into race conditions

// Usage: ./bulk-delete.js "channel:clear"
...
if (!cli.args.other || !cli.args.other.length) die("Usage: ./bulk-delete.js \"channel:clear\"\n");
...
var app = {
	...
	goDelete: function(data) {
		// perform bulk deletes
		var self = this;
		
		cli.yesno("\nDelete " + data.records.length + " records now?", "n", function(yes) {
			if (!yes) die("User abort.\n");
			
			var delete_records = [];
			data.records.forEach( function(record) {
				delete_records.push( record.seq_id );
			});
			
			verbose( JSON.stringify(delete_records, null, "\t") + "\n" );
			print("BULK DELETE GO!\n\n");
			
			self.unbase.bulkDelete( 'speech', delete_records, function(err) {
				if (err) die("Bulk Delete Error: " + err + "\n");
