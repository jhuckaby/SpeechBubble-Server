...
var storage = new StandaloneStorage(config.Storage, function(err) {
	...
	switch (cmd) {
			...
			storage.get( 'global/users', function(err) {
				...
				async.eachSeries( setup.storage,
					...
					function(err) {
						...
						print( "You should now be able to start the service by typing: '/opt/speechbubble/bin/control.sh start'\n" );
		...
		case 'export':
			// export all storage data (except sessions)
...
function export_data(file) {
	...
	var file_header = "# SpeechBubble Data Export v1.0\n" + 
		"# Hostname: " + hostname + "\n" + 
...
function import_data(file) {
	...
	// one record per line: KEY - JSON
	print( "\nSpeechBubble Data Importer v1.0\n" );
