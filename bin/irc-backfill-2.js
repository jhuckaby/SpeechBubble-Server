...
var api_key_ids = {
	GitNotify: "kjq0c8kje08",
	OnTrackNotify: "kjq0cef8109",
	ReleaseNotify: "kjq0cfkur0b",
	AutobuildNotify: "kjq0cg8mz0c",
	RollNotify: "kjq0cez6l0a",
	PerformaNotify: "kjq0chcat0e",
	DNSNotify: "kjq0cgwog0d",
	CronicleNotify: "kjq32h5j37x"
};
var api_link_titles = {
	GitNotify: "View Commit Diff",
	OnTrackNotify: "View Ticket",
	ReleaseNotify: "View Release Notes",
	AutobuildNotify: "View Build Report",
	RollNotify: "View Details",
	PerformaNotify: "View Details",
	DNSNotify: "",
	CronicleNotify: "View Details"
};
...
var app = {
	...
	run: function() {
		...
		var queue = async.queue( function(data, callback) {
			...
			else if (data.content.match(/^Snapshot\:\s+https?\:\/\/admission\./)) {
				data.tags = 'snapshot';
			}
			else if (data.content.match(/^Upload\:\s+https?\:\/\/admission\./)) {
				data.tags = 'upload';
			}
			else if (api_key_ids[nick]) {
				// API Key (App) Post
				data.type = 'app';
				data.username = api_key_ids[nick];
				
				var link_title = api_link_titles[nick] || 'Details';
				
				data.content = data.content.trim().replace(/\b(https?:\/\/\S+)$/i, '([' + link_title + ']($1))');
		...
		queue.drain = function() {
			if (last_row) return; // processing still in progress (race condition, probably in dryrun mode)
		...
		rl.on('line', function(line) {
			...
			if (line.match(/\[([\d\.]+)\]\[([\d\-\s\:]+)\]\[(\d+)\]\[transcript\]\[([\d\.]+)\]\[(\w+).+?\s+PRIVMSG\s+\#(\w+)\s+(.+)$/)) {
				...
				if (row.content.match(/\b(tracker\.eng\.admission\.net|\/cgi-bin\/ontrack\/view_record\.pl\/templates)\b/)) return;
				...
				else if (0 && (row.username == last_row.username) && (row.channel_id == last_row.channel_id) && (row.date - last_row.date <= 0.5)) {
		...
		rl.on('close', function() {
			...
			else if (!self.stats.total_inserted) server.shutdown();
		});
	},
